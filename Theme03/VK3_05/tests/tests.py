...
class TestCode(unittest.TestCase):
    ...
    def test_Python(self):
        ...
        try:
            with open('src/nimi.txt') as f:
                nimitulostus = f.read()
                rivit = nimitulostus.split("\n")
                self.assertTrue(rivit[0].count("        a") == 1)
                if (len(rivit[-1].strip()) == 0): # Jos viimeinen rivi on tyjjä, niin katsellaan onko toiseksi viimeisellä rivillä
                    self.assertTrue(rivit[-2] == "J")
                else:
                    self.assertTrue(rivit[-1] == "J")
        except:
            self.fail("Nimi.txt ei löydy")
