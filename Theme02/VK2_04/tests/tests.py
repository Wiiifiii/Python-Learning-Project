...
class TestCode(unittest.TestCase):
    ...
    def test_Python(self):
        ...
        results = callpython(cmdline_args=[], input='124\n').strip()

        expected_results = ['Oikein', "1 kerralla"]
        ...
        self.assertTrue(results.count("pienen") == 2)
        self.assertTrue(results.count("suuren") == 2)
        self.assertTrue(results.count("5 kerralla") == 1)

        mycode = loadmycode()


        self.assertTrue(mycode.count('ARVATTAVA_LUKU()') >= 1)
        self.assertTrue(mycode.count('while') >= 1)
